<template>
    <div>
        <div class="wt-tabscontenttitle wt-addnew">
            <h2>{{trans('lang.courses_in_progress')}}</h2>
            <a href="javascript:void(0);" @click="addProject" class="add-project-btn">{{trans('lang.add_course')}}</a>
        </div>
            ...
            <span v-if="stored_projects" class="project-inner-list">
                 <li v-for="(stored_project, index) in stored_projects" :key="index" class="project-element project-inner-list-item" :id="'project-element-'+index">
                    <updateProject
                        ...
                        :stored_resource_title="stored_project.resource_title"
                        :stored_resource_description="stored_project.resource_description"
                        :stored_project_img="stored_project.project_hidden_image"
                        :resource_title_name="'project['+[index]+'][resource_title]'"
                        :resource_description_name="'project['+[index]+'][resource_description]'"
                        :remove_uploded_image_id="'upload_id-'+index"
                        :uploaded_image_remove_id="'remove_upload_id-'+index"
                        :resource_title="project.resource_title"
                        :resource_description="project.resource_description"
                ...
                <li v-for="(project, index) in projects" :key="index" ref="projectlistelement" class="project-inner-list-item">
                    <div class="wt-accordioninnertitle">
                        <div :id="'projectaccordion['+project.count+']'" class="wt-projecttitle collapsed" data-toggle="collapse" :data-target="'#projectaccordioninner['+project.count+']'">
                            ...
                            <h3>{{project.resource_title}}<span>{{project.resource_description}}</span></h3>
                        </div>
                    ...
                    <div class="wt-collapseexp collapse hide" :id="'projectaccordioninner['+project.count+']'" :aria-labelledby="'projectaccordion['+project.count+']'" data-parent="#accordion">
                        <fieldset>
                            <div class="form-group form-group-half">
                                <input type="text" v-bind:name="'project['+[project.count]+'][resource_title]'" class="form-control" :placeholder="ph_course_title" v-model="project.resource_title">
                            </div>
                            <div class="form-group form-group-half">
                                <input type="text" v-bind:name="'project['+[project.count]+'][resource_description]'" class="form-control" :placeholder="ph_credit_hours_title" v-model="project.resource_description">
                            </div>
...
export default{
    ...
    props: ['widget_title', 'ph_course_title', 'ph_credit_hours_title', ],
        data(){
            return {
                start_date: '',
                end_date: '',
                ...
                project: {
                    ...
                    resource_title: this.ph_course_title,
                    project_hidden_image:'',
                    resource_description: this.ph_credit_hours_title,
                    count: 0,
                    img_id: 'profile_banner',
                    img_ref: 'profile_banner_ref',
                    preview_class:'dropzone-previews',
                    option:{
                        url: APP_URL+'/student/upload-temp-image',
                        maxFilesize: 2, // MB
                        maxFiles: 1,
