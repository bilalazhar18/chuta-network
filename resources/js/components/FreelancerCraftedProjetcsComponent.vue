    ...
    <div class="wt-projects">
        <div class="wt-project" v-if="index <= projects.length" v-for="(commentIndex, index) in commentsToShow" :key="index">
            ...
            <div class="wt-projectcontent" v-if="projects[index]">
                <h3 v-if="projects[index].resource_title">{{projects[index].resource_title}}</h3>
                <a :href="projects[index].resource_description" v-if="projects[index].resource_description">{{projects[index].resource_description}}</a>
            </div>
        </div>
