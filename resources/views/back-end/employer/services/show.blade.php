                    ...
                    <div class="wt-dashboardboxcontent wt-jobdetailsholder">
                        @if (Auth::user()->getRoleNames()[0] == 'employer')
                            <div class="wt-service-tabel wt-jobservice-details">
                                @if (!empty($student))
                                    @if (!empty($attachment))
                                        <figure class="service-feature-image"><img src="{{{asset('/uploads/services/'.$student->id.'/'.$attachment[0])}}}" alt="{{{$service['title']}}}"></figure>
                                    @endif
                                @endif
                        ...
                        @else
                            <div class="wt-studentholder wt-tabsinfo">
                        ...
                        @if (!empty($student))
                            @if (Auth::user()->getRoleNames()[0] == 'employer')
                                <div class="wt-rcvproposalholder wt-hiredstudent wt-tabsinfo">
                                    <div class="wt-tabscontenttitle">
                                        <h2>{{ trans('lang.hired_student') }}</h2>
                                    </div>
                                    <div class="wt-jobdetailscontent">
                                        <div class="wt-userlistinghold wt-featured wt-proposalitem">
                                            @if (!empty($student))
                                                ...
                                                <figure class="wt-userlistingimg">
                                                    <img src="{{{Helper::getProfileImage($student->id)}}}" alt="image description">
                                                </figure>
                                                <div class="wt-proposaldetails">
                                                    <div class="wt-contenthead">
                                                        <div class="wt-title">
                                                            <a href="{{{url('profile/'.$student->slug)}}}"> {{{Helper::getUserName($student->id)}}}</a>
                                                        </div>
                                                    </div>
                            ...
                            <div class="wt-projecthistory">
                                ...
                                <div class="wt-historycontent la-jobdetails-holder">
                                    <private-message :placeholder="'{{ trans('lang.ph_job_dtl') }}'" :upload_tmp_url="'{{url('service/upload-temp-message_attachments')}}'" :id="'{{$pivot_id}}'" :recipent_id="'{{$student->id}}'" :project_type="'service'"></private-message>
                                </div>
                            </div>
                        @endif
                    </div>
                    ...
                    <form class="wt-formtheme wt-formfeedback" id="submit-review-form">
                        <fieldset>
                            ...
                            <div class="form-group wt-btnarea">
                                <a class="wt-btn" href="javascript:void(0);" v-on:click='submitFeedback({{$student->id}}, {{$pivot_id}})'>{{ trans('lang.btn_send_feedback') }}</a>
                            </div>
                        </fieldset>
                    </form>
                </div>
