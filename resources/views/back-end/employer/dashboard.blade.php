            ...
            <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                <div class="wt-insightsitemholder">
                    <div class="row">
                        ...
                        @if($user->expiry_date>=$date && $user->expiry_date!='null')
                         <div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-3">
                            <div class="wt-insightsitem wt-dashboardbox">
                                <figure class="wt-userlistingimg">
                                   {{--  {{ Helper::getImages('uploads/settings/icon',$latest_package_expiry_icon, 'lnr lnr-heart') }} --}}
                                    <span class="lnr lnr-license"></span>
                                </figure>
                                <div class="wt-insightdetails">
                                    <div class="wt-title">
                                        <h3>{{$package_item->item_name}}</h3>
                                        <a href="javascript:void(0)">{{$package_item->item_name}} Activated</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        
                        @elseif (!empty($enable_package) && $enable_package === 'true')
                            @if (!empty($package))
                                <div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-3">
                        ...
                        <div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-3">
                            <div class="wt-insightsitem wt-dashboardbox">
                                ...
                                <div class="wt-insightdetails">
                                    <div class="wt-title">
                                        <h3>{{ trans('lang.view_saved_students') }}</h3>
                                ...
                                <table class="wt-tablecategories wt-student-table">
                                    ...
                                    <tbody>
                                        @foreach ($ongoing_jobs as $project)
                                            @php
                                                $proposal_student = $project->proposals->where('status', 'hired')->pluck('freelancer_id')->first();
                                                $student = !empty($proposal_student) ? \App\User::find($proposal_student) : ''; 
                                                $user_name = Helper::getUsername($proposal_student);
                                            @endphp
                                            <tr>
                                                <td data-th="Project title"><span class="bt-content"><a target="_blank" href="{{{ url('job/'.$project->slug) }}}">{{{ $project->title }}}</a></span></td>
                                                @if (!empty($student))
                                                    <td data-th="Hired student">
                                                        <span class="bt-content">
                                                            <a href="{{{url('profile/'.$student->slug)}}}">
                                                                @if ($student->user_verified)
