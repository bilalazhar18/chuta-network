                ...
                <div class="wt-dashboardbox wt-dashboardtabsholder wt-saveitemholder">
                    <div class="wt-dashboardtabs">
                        <ul class="wt-tabstitle nav navbar-nav">
                            ...
                            <li class="nav-item"><a data-toggle="tab" href="#wt-awards">{{ trans('lang.liked_students') }}</a></li>
                        </ul>
                    </div>
                    <div class="wt-tabscontent tab-content tab-savecontent">
                        ...
                        <div class="wt-awardsholder tab-pane fade" id="wt-awards">
                            <div class="wt-addprojectsholder wt-likefreelan la-likefreelanfav">
                                <div class="wt-tabscontenttitle">
                                    <h2>{{ trans('lang.liked_students') }}</h2>
                                </div>
                                @if (!empty($saved_students))
                                <div class="wt-likedstudents wt-haslayout">
                                    @foreach ($saved_students as $key => $student)
                                        @php
                                            $user = \App\User::find($student);
                                            $profile = \App\User::find($student)->profile;
                                           
                                            $rating = !empty($profile->rating) ? $profile->rating : 0;
                                            $user_image = !empty($profile) ? $profile->avater : '';
                                            $profile_image = !empty($user_image) ? '/uploads/users/'.$student.'/'.$user_image : 'images/user.jpg';
                                            $user_name = Helper::getUserName($student);
                                            $reviews = \App\Review::where('receiver_id', $student)->count();
                                            ...
                                            $verified_user = \App\User::select('user_verified')->where('id', $student)->pluck('user_verified')->first();
                            ...
                            @if ( method_exists($saved_students,'links') )
                                {{ $saved_students->links('pagination.custom') }}
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <aside id="wt-sidebar" class="wt-sidebar wt-dashboardsave">
                    ...
                    <div class="wt-proposalsr">
                        <div class="wt-proposalsrcontent  wt-freelancelike">
                            ...
                            <div class="wt-title">
                                <h3>{{{ count($saved_students) }}}</h3>
                                <span>{{ trans('lang.students_liked') }}</span>
                            </div>
                        </div>
                    </div>
                </aside>
