                        ...
                        <div class="wt-dashboardboxcontent">
                            {!! Form::open([ 'url' => url('admin/store/package'), 'class' =>'wt-formtheme wt-formprojectinfo wt-formcategory', 'id' => 'packages', '@submit.prevent' => 'submitPackage', 'id' => 'package_form' ]) !!}
                                <fieldset>
                                    ...
                                    <div v-if="student_options" v-cloak>
                                        <div class="form-group">
                                            {!! Form::text('student[no_of_connects]', null, array('class' => 'form-control', 'placeholder' => trans('lang.no_of_connects'), 'v-model'=>'package.conneects')) !!}
                                        </div>
                                        <div class="form-group">
                                            {!! Form::number( 'student[no_of_services]', null, ['class' =>'form-control ', 'placeholder' => trans('lang.student_pkg_opt.no_of_services'), 'v-model'=>'package.services'] ) !!}
                                        </div>
                                        <div class="form-group">
                                            {!! Form::number( 'student[no_of_featured_services]', null, ['class' =>'form-control ', 'placeholder' => trans('lang.student_pkg_opt.no_of_featured_services'), 'v-model'=>'package.featured_services'] ) !!}
                                        </div>
                                        <div class="form-group">
                                            {!! Form::text('student[no_of_skills]', null, array('class' => 'form-control', 'placeholder' => trans('lang.no_of_skills'), 'v-model'=>'package.skills')) !!}
                                        </div>
                                        <div class="form-group">
                                            <span class="wt-select">
                                                <select name="student[duration]">
                                                    <option value="" disabled="">{{ trans('lang.select_duration') }}</option>
                                                    @foreach ($durations as $key => $duration)
                                                        <option value="{{$key}}">{{ Helper::getPackageDurationList($key) }}</option>
                                                    @endforeach
                                        ...
                                        <div class="form-group">
                                            <span class="wt-select">
                                                {!! Form::select('student[badge]', $badges, null, array('placeholder' => trans('lang.select_badge'))) !!}
                                            </span>
                                        </div>
                                        <div class="form-group">
                                            <switch_button v-model="banner_option">{{{ trans('lang.show_banner_opt') }}}</switch_button>
                                            <input type="hidden" :value="banner_option" name="student[banner_option]">
                                        </div>
                                        <div class="form-group">
                                            <switch_button v-model="private_chat">{{{ trans('lang.enabale_disable_pvt_chat') }}}</switch_button>
                                            <input type="hidden" :value="private_chat" name="student[private_chat]">
                                        </div>
                                        @if ($student_trial->count() == 0)
                                            <div class="form-group">
                                                <span class="wt-checkbox">
                                                    <input id="trial" type="checkbox" name="trial">
                                                    <label for="trial"><span>{{{ trans('lang.enable_trial') }}}</span></label>
