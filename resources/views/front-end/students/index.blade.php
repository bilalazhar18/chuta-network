                                ...
                                $user_image = !empty($student->profile->avater) ?
                                        ...
                                        <div class="wt-userlistinghold {{ $feature_class }}">
                                            ...
                                            <figure class="wt-userlistingimg">
                                                ...
                                                @else
                                                <img src="{{{ asset(Helper::getImageWithSize('uploads/users/'.$student->user_id, $student->avater, 'listing')) }}}" alt="{{ trans('lang.img') }}">
                                                @endif
                                            </figure>
                                                ...
                                                <div class="wt-contenthead">
                                                    <div class="wt-title">
                                                        <a href="{{{ url('profile/'.$student->slug) }}}">
                                                            ...
                                                            @endif
                                                            {{{ Helper::getUserName($student->id) }}}
                                                        </a>
                                                        @if (!empty($student->profile->tagline))
                                                            <h2><a href="{{{ url('profile/'.$student->slug) }}}">{{{ $student->profile->tagline }}}</a></h2>
                                                        @endif
                                                    </div>
                                                    <ul class="wt-userlisting-breadcrumb">
                                                        @if (!empty($student->profile->cgpa))
                                                            <li><span><i class="fas fa-graduation-cap"></i>
                                                                {{{ $student->profile->cgpa }}}</span>
                                                            </li>
                                                        @endif
                                                        @if (!empty($student->location))
                                                            <li><span><img src="{{{ asset($flag)}}}" alt="Flag"> {{{ !empty($student->location->title) ? $student->location->title : '' }}}</span></li>
                                                        @endif
                                                        ...
                                                        @else
                                                            <li v-cloak>
                                                                <a class="wt-clicklike" id="student-{{$student->id}}" @click="add_wishlist('student-{{$student->id}}', {{$student->id}}, 'saved_student', '{{trans("lang.saved")}}')">
                                                                    <i class="fa fa-heart"></i>
                                            ...
                                            @if (!empty($student->profile->description))
                                                <div class="wt-description">
                                                    <p>{{{ str_limit($student->profile->description, 180) }}}</p>
                                                </div>
                                            @endif
