...
class Job extends Model
{
    ...
    public function storeJobs($request)
    {
        ...
        if (!empty($request)) {
            ...
            $this->course_id = $skills;
            //$cat=implode(',',$request['categories']);
            $this->title = filter_var($request['title'], FILTER_SANITIZE_STRING);
            $this->slug = filter_var($request['title'], FILTER_SANITIZE_STRING);
            
            $this->category = filter_var($request['categories'], FILTER_SANITIZE_STRING);
            $this->universityapp = filter_var($request['universityapp'], FILTER_SANITIZE_STRING);
            $this->worktype = filter_var($request['worktype'], FILTER_SANITIZE_STRING);
            $this->paytype = filter_var($request['paytype'], FILTER_SANITIZE_STRING);
            $this->payrange  = filter_var($request['payrange'], FILTER_SANITIZE_STRING);
            $this->degree_id = filter_var($request['degreearea'], FILTER_SANITIZE_STRING);
            $this->business_details = $request['business_details'];
            $this->faculty = $request['faculty'];
            $this->description = $request['description'];
            $this->location_id = filter_var($request['locations'], FILTER_SANITIZE_STRING);
             $this->expiry_date=date("Y-m-d",strtotime(Auth::user()->expiry_date));
            ...
            // $this->student_type = filter_var($request['student_type'], FILTER_SANITIZE_STRING);
    ...
    public function updateJobs($request, $id)
    {
        ...
        if (!empty($request)) {
            ...
            $job->category = filter_var($request['categories'], FILTER_SANITIZE_STRING);
            $job->universityapp = filter_var($request['universityapp'], FILTER_SANITIZE_STRING);
            $job->worktype = filter_var($request['worktype'], FILTER_SANITIZE_STRING);
            $job->paytype = filter_var($request['paytype'], FILTER_SANITIZE_STRING);
            $job->payrange  = filter_var($request['payrange'], FILTER_SANITIZE_STRING);
            $job->degree_id = filter_var($request['degreearea'], FILTER_SANITIZE_STRING);
            $job->business_details = $request['business_details'];
             $job->faculty = $request['faculty'];
            $job->location_id = filter_var($request['locations'], FILTER_SANITIZE_STRING);
            $job->description = $request['description'];
            $skills=implode(',',$request['skills']);
            $job->course_id = $skills;
 ...
 public static function getSearchResult(
        ...
        // }
            ...
            $filters['category'] = $search_categories;
            foreach ($search_categories as $key => $search_category) {
                $categor_obj = Degree::where('title', $search_category)->first();
                $category = Degree::find($categor_obj->id);
                if (!empty($category->jobs)) {
                    $category_jobs = $category->jobs->pluck('id')->toArray();
                    foreach ($category_jobs as $id) {
                        $job_id[] = $id;
       ...
       // echo "hello";die;
        if (!empty($search_degree)) {
            ...
            $filters['degree'] = $search_degree;
            foreach ($search_degree as $key => $search_degree) {
                $categor_obj = Degree::where('title', $search_degree)->first();
                $category = Degree::find($categor_obj->id);
                if (!empty($category)) {
