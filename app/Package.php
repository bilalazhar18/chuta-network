    ...
    public function savePackage($request)
    {
        if (!empty($request)) {
            ...
            if ($request->roles == '2') {
                ...
                $this->options = serialize($request['student']);
            }
            ...
            $this->badge_id = !empty($request['student']['badge']) ? intval($request['student']['badge']) : 0;
            return $this->save();
        }
    }
    ...
    public function updatePackage($request, $slug)
    {
        if (!empty($request)) {
            ...
            if ($request->roles == '2') {
                ...
                $package->options = serialize($request['student']);
            }
            $package->badge_id = !empty($request['student']['badge']) ? intval($request['student']['badge']) : 0;
            $package->save();
        }
    }
