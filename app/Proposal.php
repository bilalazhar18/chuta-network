...
class Proposal extends Model
{
    ...
    public function storeProposal($request, $id, $login_id ='')
    {
        ...
        if (!empty($request)) {
            ...
            $this->experience = filter_var($request['experience'], FILTER_SANITIZE_STRING);
            $this->course = filter_var($request['course'], FILTER_SANITIZE_STRING);
            ...
            // $filename=str_replace(" ", "_", $request['attachments']);
             $filename1 = md5($request['attachments']->getFilename().time()).'.'.$request['attachments']->getClientOriginalExtension();
            $this->attachments = 'https://chuta.network/dev/public'.'/uploads/proposals/'.Auth::user()->id.'/'.$filename1;
            $path = 'https://chuta.network/dev/public'.'/uploads/proposals/'.Auth::user()->id.'/';
            $uploadedFile = $request['attachments'];
            $uploadedFile->move($path,$filename1);
     ...
     * @param int   $freelancer_id student ID
    ...
    public function getJobSkillRequirement($job_skills)
    {
        ...
        $skill = '';
        if (!empty($job_skills)) {
            $student_skills = auth()->user()->skills->pluck('id')->toArray();
            foreach ($job_skills as $key => $skill) {
                if (!(in_array($skill, $student_skills))) {
                    $json[$key] = $skill;
                }
     ...
     * @return \Illuminate\Http\Response
     */
    public function getstudentsTotalAmount()
    {
