...
class Review extends Model
{
    ...
    public static function submitReview($request, $author_id, $project_type)
    {
        ...
        $student_total_rating =  Self::select('avg_rating')
            ->where('receiver_id', $request['receiver_id'])->sum('avg_rating');
        $student_rating = $student_total_rating / Self::where('receiver_id', $request['receiver_id'])->count();
        if (!empty($student_rating)) {
            $rating = array();
            $student_profile = Profile::find($request['receiver_id']);
            $rating = unserialize($student_profile->rating);
            $rating = !empty($rating) && is_array($rating) ? $rating : array();
            $rating[] = round($student_rating);
            $rating = array_unique($rating);
            $student_profile->ratings = serialize($rating);
