...
class UserController extends Controller
{
    ...
    public function getSavedItems(Request $request, $role = '')
    {
        if (Auth::user()) {
            ...
            $saved_students = !empty($profile->saved_student) ? unserialize($profile->saved_student) : array();
            ...
            } elseif ($request->path() === 'student/saved-items') {
                if (file_exists(resource_path('views/extend/back-end/student/saved-items.blade.php'))) {
                    return view(
                        'extend.back-end.student.saved-items',
                        compact(
                            'profile',
                            'saved_jobs',
                            'saved_students',
        ...
        if (Auth::user()) {
            ...
            if (!empty($request['slug'])) {
                ...
                if ($role->role_type == 'student') {
                    ...
                    if (in_array($selected_user->id, unserialize($profile->saved_student))) {
                        $json['current_student'] = 'true';
                    }
                    return $json;
    ...
    public function addWishlist(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            if (!empty($request['id'])) {
                $user_id = Auth::user()->id;
                $id = $request['id'];
                if (!empty($request['column']) && ($request['column'] === 'saved_employers' || $request['column'] === 'saved_student' || $request['column'] === 'saved_services')) {
    ...
    public function submitReview(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            if ($submit_review['type'] == "success") {
                ...
                //send email
                if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                    $student = User::find($request['receiver_id']);
                    $email_params = array();
                    ...
                    if ($project_type == 'job') {
                        ...
                        //$student = Proposal::select('freelancer_id')->where('status', 'completed')->first();
                        ...
                        $student_job_completed_template = DB::table('email_types')->select('id')->where('email_type', 'student_email_job_completed')->get()->first();
                        if (!empty($student_job_completed_template->id)) {
                            $template_data = EmailTemplate::getEmailTemplateByID($student_job_completed_template->id);
                            Mail::to($student->email)
                                ->send(
                                    new studentEmailMailable(
                                        'student_email_job_completed',
                                        $template_data,
                                        $email_params
                                    )
                                );
                        ...
                        $email_params['resource_title'] = $service->title;
                        $email_params['completed_project_link'] = url('service/' . $service->slug);
                        $template_data = Helper::getstudentCompletedServiceEmailContent();
                        Mail::to($student->email)
                            ->send(
                                new studentEmailMailable(
                                    'student_email_job_completed',
        ...
        if (Auth::user()) {
            ...
            if ($report == 'success') {
                ...
                if (
                    ...
                    || $request['report_type'] == 'student-report'
                ) {
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        $email_params = array();
                        ...
                        } else if ($request['report_type'] == 'student-report') {
                            $report_student_template = DB::table('email_types')->select('id')->where('email_type', 'admin_email_report_student')->get()->first();
                            if (!empty($report_student_template->id)) {
                                $student = User::find($request['id']);
                                $template_data = EmailTemplate::getEmailTemplateByID($report_student_template->id);
                                $email_params['reported_student'] = Helper::getUserName($request['id']);
                                $email_params['link'] = url('profile/' . $student->slug);
                                $email_params['report_by_link'] = url('profile/' . $user->slug);
                                $email_params['reported_by'] = Helper::getUserName(Auth::user()->id);
                                $email_params['message'] = $request['description'];
                    ...
                    $student_job_cancelled = DB::table('email_types')->select('id')->where('email_type', 'student_email_cancel_job')->get()->first();
                    $json['message'] = trans('lang.job_cancelled');
                    if (!empty($student_job_cancelled->id)) {
                        if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                            $template_data = EmailTemplate::getEmailTemplateByID($student_job_cancelled->id);
                            $job = Job::find($request['id']);
            ...
            if (!empty($request['id']) && !empty($request['status'])) {
                ...
                if ($order->type == 'job') {
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        $email_params['student_name'] = Helper::getUserName($service->seller[0]->id);
                        $email_params['employer_profile'] = url('profile/' . $user->slug);
                        $email_params['employer_name'] = Helper::getUserName($user->id);
                        $student_data = User::find(intval($service->seller[0]->id));
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if ($role === 'employer') {
                            ...
                            if (!empty($user->email)) {
                                ...
                                if (!empty($template->id)) {
                                    $template_data = EmailTemplate::getEmailTemplateByID($template->id);
                                    $email_params['student'] = Helper::getUserName($user->id);
     ...
     * @return \Illuminate\Http\Response
     */
    public function getstudentInvoices($type = '')
    {
        if (Auth::user()->getRoleNames()[0] != 'admin' && Auth::user()->getRoleNames()[0] === 'student') {
            $invoices = array();
            $invoices = DB::table('invoices')
                ->join('items', 'items.invoice_id', '=', 'invoices.id')
                ->join('packages', 'packages.id', '=', 'items.product_id')
            ...
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? Helper::currencyList($currency[0]['currency']) : array();
            if ($type === 'project') {
                if (file_exists(resource_path('views/extend/back-end/student/invoices/project.blade.php'))) {
                    return view('extend.back-end.student.invoices.project', compact('invoices', 'type', 'expiry_date', 'symbol'));
                } else {
                    return view('back-end.student.invoices.project', compact('invoices', 'type', 'expiry_date', 'symbol'));
                }
            } elseif ($type === 'package') {
                if (file_exists(resource_path('views/extend/back-end/student/invoices/package.blade.php'))) {
                    return view('extend.back-end.student.invoices.package', compact('invoices', 'type', 'expiry_date', 'symbol'));
                } else {
                    return view('back-end.student.invoices.package', compact('invoices', 'type', 'expiry_date', 'symbol'));
                }
            }
        } else {
            abort(404);
    ...
    public function showInvoice($id)
    {
        if (!empty($id)) {
            ...
            if (Auth::user()->getRoleNames()->first() === 'student') {
                if (file_exists(resource_path('views/extend/back-end/student/invoices/show.blade.php'))) {
                    return view::make('extend.back-end.student.invoices.show', compact('invoice_info', 'symbol', 'currency_code'));
                } else {
                    return view::make('back-end.student.invoices.show', compact('invoice_info', 'symbol', 'currency_code'));
                }
        ...
        if (!empty($request)) {
            ...
            if (Auth::user()->getRoleNames()->first() === 'student') {
                ...
                if ($storeProjectOffers == "success") {
                    ...
                    //         $f_link = url('profile/' . $student->slug);
                    //         $f_name = Helper::getUserName($student->id);
                    //         $e_name = Helper::getUserName(Auth::user()->id);
                    //         $e_link = url('profile/' . $user->slug);
                    //         $p_link = "Friend Request";
                    ...
                    //         $message->is_friend = 0;
                    //         $message->save();
                    //         $email_params['resource_title'] = $p_title;
    ...
    public function raiseDispute($slug)
    {
        ...
        if (file_exists(resource_path('views/extend/back-end/student/jobs/dispute.blade.php'))) {
            return View(
                'extend.back-end.student.jobs.dispute',
                compact(
                    'job',
                    'reasons',
                    'show_breadcrumbs'
