...
class EmployerController extends Controller
{
    ...
    public function showServiceDetail($service_id, $pivot_id, $status)
    {
        if (Auth::user()) {
            ...
            // $avg_rating = Review::where('receiver_id', $student->id)->sum('avg_rating');
            ...
            $reviews = Review::where('receiver_id', $student->id)->get();
            ...
            $reviews = !empty($student) ? Review::where('receiver_id', $student->id)->where('job_id', $service_id)->where('project_type', 'service')->get() : '';
    ...
    public function employerPaymentProcess($id)
    {
        if (Auth::user() && !empty($id)) {
            if (Auth::user()) {
                ...
                $student_name = Helper::getUserName($proposal->freelancer_id);
