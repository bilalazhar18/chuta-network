...
class RestAPIController extends Controller
{
    ...
    public function getstudent()
    {
        ...
        if (!empty($login_user) && !empty($type)) {
            $user_by_role =  User::role('student')->select('id')
            ...
            if ($type === 'featured') {
                ...
                if (!empty($subscribers)) {
                    ...
                    $json['type']        = 'error';
                    $json['message']    = trans('lang.no_students_list');
                ...
                $save_student = !empty($login_user->profile->saved_student) ?
                    unserialize($login_user->profile->saved_student) : array();
                if (!empty($save_student)) {
                    $users =  $users->whereIn('id', $save_student)->orderBy('updated_at', 'desc')
                        ->paginate($post_per_page);
                } else {
                    $json['type']        = 'error';
                    $json['message']    = trans('lang.no_students_list');
                ...
                if (!empty($search_skills)) {
                    ...
                    foreach ($skills as $key => $skill) {
                        if (!empty($skill->students[$key]->id)) {
                            $user_id[] = $skill->students[$key]->id;
                        }
                    }
                ...
                if (!empty($search_freelaner_types)) {
                    ...
                    $students = Profile::whereIn('student_type', $search_freelaner_types)->get();
                ...
                if (!empty($search_english_levels)) {
                    ...
                    $students = Profile::whereIn('english_level', $search_english_levels)->get();
                    foreach ($students as $key => $student) {
                        if (!empty($student->user_id)) {
                            $user_id[] = $student->user_id;
                        }
            ...
            if (!empty($users) && $users->count() > 0) {
                foreach ($users as $key => $user) {
                    ...
                    $json[$key]['favorit'] = in_array($user['id'], $save_student) ? 'yes' : '';
                    $json[$key]['name'] = Helper::getUserName($user['id']);
                    $json[$key]['student_link'] = url('profile/' . $user_object->slug);
                    ...
                    $student_educations  = array();
                    if (!empty($educations)) {
                        foreach ($educations as $edu_key => $education) {
                            $student_educations[$edu_key]['title'] = !empty($education['degree_title']) ? $education['degree_title'] : '';
                            $student_educations[$edu_key]['institute'] = !empty($education['institute_title']) ? $education['institute_title'] : '';
                            $student_educations[$edu_key]['startdate'] = !empty($education['start_date']) ? url($education['start_date']) : '';
                            $student_educations[$edu_key]['enddate'] = !empty($education['end_date']) ? url($education['end_date']) : '';
                            $student_educations[$edu_key]['description'] = !empty($education['description']) ? url($education['description']) : '';
                        }
                        $json[$key]['_educations'] = $student_educations;
                    }

                    $experiences = !empty(unserialize($user_object->profile->experience)) ? unserialize($user_object->profile->experience) : array();
                    $student_experiences  = array();
                    if (!empty($experiences)) {
                        foreach ($experiences as $exp_key => $experience) {
                            $student_experiences[$exp_key]['title'] = !empty($experience['job_title']) ? $experience['job_title'] : '';
                            $student_experiences[$exp_key]['company'] = !empty($experience['company_title']) ? $experience['company_title'] : '';
                            $student_experiences[$exp_key]['startdate'] = !empty($experience['start_date']) ? url($experience['start_date']) : '';
                            $student_experiences[$exp_key]['enddate'] = !empty($experience['end_date']) ? $experience['end_date'] : '';
                            $student_experiences[$exp_key]['description'] = !empty($experience['description']) ? $experience['description'] : '';
                        }
                        $json[$key]['_experiences'] = $student_experiences;
                    }
                    ...
                    if (!empty($projects)) {
                        foreach ($projects as $project_key => $project) {
                            $student_projects[$project_key]['title'] = !empty($project['resource_title']) ? $project['resource_title'] : '';
                            $student_projects[$project_key]['url'] = !empty($project['resource_description']) ? $project['resource_description'] : '';
                            $student_projects[$project_key]['image']['url'] = !empty($project['project_hidden_image']) ? url('/uploads/users/' . $user['id'] . '/projects/' . $project['project_hidden_image']) : '';
                        }
                        $json[$key]['_projects'] = $student_projects;
                    }
                    ...
                    if (!empty($awards)) {
                        foreach ($awards as $award_key => $award) {
                            $student_awards[$award_key]['title'] = !empty($award['award_title']) ? $award['award_title'] : '';
                            $student_awards[$award_key]['date'] = !empty($award['award_date']) ? $award['award_date'] : '';
                            $student_awards[$award_key]['image']['url'] = !empty($award['award_hidden_image']) ? url('/uploads/users/' . $user['id'] . '/awards/' . $award['award_hidden_image']) : '';
                        }
                        $json[$key]['_awards'] = $student_awards;
                    }
                    ...
                    $json[$key]['_student_type'] = !empty($user_object->profile->student_type) ? $user_object->profile->student_type : '';
                    ...
                    if (!empty($reviews) && $reviews->count() > 0) {
                        foreach ($reviews as $reviews_key => $review) {
                            ...
                            $student_reviewes[$reviews_key]['resource_title'] = !empty($job->title) ? $job->title : '';
                            $student_reviewes[$reviews_key]['post_date'] = Carbon::parse($job->created_at)->format('M Y');
                            $student_reviewes[$reviews_key]['employer_image'] = !empty(Helper::getProfileImage($review->user_id)) ? asset(Helper::getProfileImage($review->user_id)) : '';
                            $student_reviewes[$reviews_key]['_is_verified'] = $verified_user == 1 ? 'yes' : 'no';
                            $student_reviewes[$reviews_key]['employer_name'] = Helper::getUserName($review->user_id);
                            $student_reviewes[$reviews_key]['level_title'] = !empty($job->project_level) ?  Helper::getProjectLevel($job->project_level) : '';
                            $student_reviewes[$reviews_key]['project_location'] = $job->location->title;
                            $student_reviewes[$reviews_key]['project_rating'] = "" . $review->avg_rating . "";
                            $student_reviewes[$reviews_key]['review_content'] = !empty($review->feedback) ?  $review->feedback : '';
                        }
                        $json[$key]['reviews'] = $student_reviewes;
                    }
                }
                    ...
                    if ($proposals <= $allowed_proposals) {
                        ...
                        if (!empty($job->employer->email)) {
                            ...
                            $proposal_submitted_template = DB::table('email_types')->select('id')
                                ->where('email_type', 'student_email_new_proposal_submitted')
                                ->get()->first();
                            if (
                                ...
                                $email_params['student'] = Helper::getUserName($user_id);
                                $email_params['student_profile'] = url('profile/' . $user->slug);
                                ...
                                Mail::to($user->email)
                                    ->send(
                                        new studentEmailMailable(
                                            'student_email_new_proposal_submitted',
            ...
            foreach ($users as $key => $user) {
                ...
                if (!empty($awards)) {
                    foreach ($awards as $award_key => $award) {
                        $student_awards[$award_key]['title'] = !empty($award['award_title']) ? $award['award_title'] : '';
                        $student_awards[$award_key]['date'] = !empty($award['award_date']) ? $award['award_date'] : '';
                        $student_awards[$award_key]['image']['url'] = !empty($award['award_hidden_image']) ? url($award['award_hidden_image']) : '';
                    }
                    $json[$key]['_awards'] = $student_awards;
                }
                $json[$key]['location']['_country'] = !empty($user_object->location->title) ? $user_object->location->title : '';
        ...
        if (!empty($user) && $user->count() > 0) {
            if (!empty($jobs)) {
                foreach ($jobs as $key => $job) {
                    ...
                    $json[$key]['resource_title'] = !empty($job->title) ? $job->title : '';
    ...
    public function updateProfile(Request $request)
    {
        ...
        if ($packages > 0) {
            ...
            $profile->student_type = !empty($request['student_type']) ?
                filter_var($request['student_type'], FILTER_SANITIZE_STRING) : $profile->student_type;
    ...
    public function listings()
    {
        ...
        if (!empty($type)) {
            ...
            if ($type === 'student_level') {
                $student_level = Helper::getstudentLevelList();
                if (!empty($student_level)) {
                    $count = 0;
                    foreach ($student_level as $value => $level) {
                        $json[$count]['title'] = !empty($level) ? Helper::getstudentLevelList($value) : '';
                        $json[$count]['value'] = !empty($level) ? $value : '';
                        $count++;
                    }
                    return Response::json($json, 200);
    ...
    public function sendProjectOffers(Request $request)
    {
        ...
        if (!empty($request['user_id'])
            ...
            if ($json['type'] == "success") {
                ...
                if (!empty($email_settings) && !empty($email_settings[0]['email'])) {
                    $email_params = array();
                    $send_student_offer = DB::table('email_types')->select('id')->where('email_type', 'student_email_send_offer')->get()->first();
                    $message = new Message();
                    if (!empty($send_student_offer->id)) {
                        $job = Job::where('id', $request['job_id'])->first();
                        $student = User::find($request['freelancer_id']);
                        $template_data = EmailTemplate::getEmailTemplateByID($send_student_offer->id);
    ...
    public function postJob(Request $request)
    {
        ...
        $student_type = !empty($request['student_level']) ? $request['student_level'] : null;
        ...
        if (!empty($package) && $current_date <= $expiry_date) {
            ...
            if ($posted_jobs >= intval($option['jobs'])) {
                ...
                $job->student_type = filter_var($student_type, FILTER_SANITIZE_STRING);
