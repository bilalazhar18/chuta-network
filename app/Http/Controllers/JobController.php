...
class JobController extends Controller
{
    ...
    public function postJob()
    {
        ...
        // add Bilal
        $worktype = Helper::getWorkTypeList();
        $paytype = Helper::getPayTypeList();
        $categories = Helper::getcategoryList();
         $faculty = Helper::getfacultyList();
        $degree=DB::table('degrees')->select('id','title')->get();
        $item=Item::select('subscriber','item_name')->where('item_name','Package 2')->where('subscriber',Auth::user()->id)->first();
        //dd($item);
   ...
   public function index()
    {
        ...
        if (file_exists(resource_path('views/extend/back-end/employer/jobs/index.blade.php'))) {
            ...
            return view('back-end.employer.jobs.index', compact('job_details', 'symbol','date'));
        }
    }
   ...
   public function edit($job_slug)
    {
        ...
        if (!empty($job_slug)) {
            $job = Job::where('id', $job_slug)->first();
             $item=Item::select('subscriber','item_name')->where('item_name','Package 2')->where('subscriber',Auth::user()->id)->first();
            $json = array();
        $languages = Language::pluck('title', 'id');
        //$locations = Location::pluck('title', 'id');
       ...
       // dd($worktype);
        ...
        $faculty = Helper::getfacultyList();
        $degree=DB::table('degrees')->select('id','title')->get();
        $skills = Skill::select('title', 'id')->get();
        //$categories = Category::pluck('title', 'id');
        ...
        $options = !empty($package_options) ? unserialize($package_options['options']) : array();
            ...
            // $english_levels = Helper::getEnglishLevelList();
            // $job_duration = Helper::getJobDurationList();
            // $student_level_list = Helper::getstudentLevelList();
    ...
    public function store(Request $request)
    {
        ...
        $user = User::select('id','expiry_date')->where('id', Auth::user()->id)->first();
        $date=date('Y-m-d');
        
        if ($user->expiry_date < $date) {
                $json['type'] = 'error';
                $json['message'] = trans('lang.package_expire');
                return $json;
            }
        else
        {
                ...
                if ($job_post = 'success') {
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if (!empty($new_posted_job_template->id) || !empty(new_posted_job_template_employer)) {
                            ...
                            $email_params['posted_job_link'] = url('job/view-jobdetails/'.$job->id);
    ...
    public function show($slug)
    {
      $job = $this->job::all()->where('slug', $slug)->first();
       // $job = $this->job::all()->where('id', $slug)->first();
