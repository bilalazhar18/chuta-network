...
class SkillController extends Controller
{
    ...
    //     $student_skills = Skill::getstudentSkill(Auth::user()->id);
    //     $result = array_diff($db_skills, $student_skills);
    //     if (!empty($result)) {
     ...
     * @return \Illuminate\Http\Response
     */
    public function getBookedMeetings()
    {
        $json = array();
        $user_id = Auth::user()->id;

        $sent_meetings = Meeting::getMeetingsSent($user_id);
        $received_meetings = Meeting::getMeetingsReceived($user_id);
        Log::info("SENT: ");
        Log::info($sent_meetings);
        Log::info("RECEIVED: ");
        Log::info($received_meetings);
        // Log::info("Mayday! Got it!");
        if (!empty($sent_meetings) || !empty($received_meetings)) {
            $json['type'] = 'success';
            $json['meetings'] = array_merge($sent_meetings, $received_meetings);
            ...
            $json['type'] = 'error';
            $json['message'] = trans('lang.something_wrong');
            // Log::info("You don't have any meeting booked");
            // Log::info($json);
            return $json;
        }
    }    
