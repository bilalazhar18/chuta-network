...
class ProposalController extends Controller
{
    ...
    {
        if (!empty($job_slug)) {
            $job = Job::all()->where('id', $job_slug)->first();
            if (!empty($job)) {
                $job_skills = $job->skills->pluck('id')->toArray();
                 $profile=Profile::select('id','cgpa','degree_id','experience','university')->where('user_id',Auth::user()->id)->get();
                 
                  $experience = !empty($profile[0]->experience) ? unserialize($profile[0]->experience) : array();
               // dd($experience);
                $degid=$profile[0]->degree_id;
                $degree = DB::table('degrees')->select('id','title')->where('id',$degid)->first();
                        ...
                        if ($submit_propsal = 'success') {
                            ...
                            if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                                if (!empty($job->employer->email)) {
                                    ...
                                    $proposal_submitted_template = DB::table('email_types')->select('id')->where('email_type', 'student_email_new_proposal_submitted')->get()->first();
    ...
    public function hiredFreelencer(Request $request)
    {
        ...
        if (!empty($request['id'])) {
            ...
            $json['type'] = 'success';
            $json['message'] = trans('lang.student_hire');
            return $json;
        }
    }
    ...
    public function show($slug)
    {
        ...
        $accepted_proposal = Job::find($job->id)->proposals()->where('hired', 1)
            ->first();
        $student_name = Helper::getUserName($accepted_proposal->freelancer_id);
