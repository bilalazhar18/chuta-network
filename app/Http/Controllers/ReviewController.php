...
class ReviewController extends Controller
{
    ...
    {
        $review_options = Resourcefeedback::paginate(10);
        if (file_exists(resource_path('views/extend/back-end/admin/review-options/index.blade.php'))) {
            return View::make(
                'extend.back-end.admin.review-options.feed',
                compact('review_options')
            );
        } else {
            return View::make(
                'back-end.admin.review-options.feed',
                compact('review_options')
            );
        }
    }
     ...
     public function deletefeed(Request $request)
    {
        ...
        $id = $request['id'];
        if (!empty($id)) {
            Resourcefeedback::where('id', $id)->delete();
            $json['type'] = 'success';
            return $json;
        }
    }
