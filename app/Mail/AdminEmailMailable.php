...
class AdminEmailMailable extends Mailable
{
    ...
    public function build()
    {
        ...
        } elseif ($this->type == 'admin_email_report_student') {
            $email_message = $this->prepareAdminEmailReportstudent($this->email_params);
     ...
     * @return string
     */
    public function prepareAdminEmailReportstudent($email_params)
    {
        extract($email_params);
        ...
        $app_content = $this->template->content;
        $email_content_default =    "Hello,
                                    A student <a href='%link%'>%reported_student%</a> has been reported by <a href='%report_by_link%'>%reported_by% </a>
                                    Message is given below.
                                    %message%
        ...
        $app_content = str_replace("%reported_student%", $student, $app_content);
        $app_content = str_replace("%link%", $student_link, $app_content);
        $app_content = str_replace("%report_by_link%", $reported_by_link, $app_content);
        $app_content = str_replace("%reported_by%", $report_by, $app_content);
        $app_content = str_replace("%message%", $report_message, $app_content);
        $app_content = str_replace("%signature%", $signature, $app_content);
    ...
    public function prepareAdminEmailServicePosted($email_params)
    {
        ...
        $email_content_default =    "Hello,
                                    A new service is posted by <a href='%student_link%'>%student_name%</a>.
    ...
    public function prepareAdminEmailJobCompleted($email_params)
    {
        ...
        $email_content_default =    "Hello,
                                    ...
                                    The <a href='%student_link%'>%student_name%</a> has completed the following project (<a href='%project_link%'>%resource_title%</a>).
    ...
    public function prepareAdminEmailDisputeRaised($email_params)
    {
        ...
        $link = $project_link;
        $student_name = $name;
        $student_link = $sender_link;
        $message = $msg;
        $dispute_reason = $reason;
        $signature = EmailHelper::getSignature();
        $app_content = $this->template->content;
        $email_content_default =   "Hello,
                                    A dispute has been raised by a student <a href='%student_link%'> %student_name% </a> against <a href='%project_link%'>%resource_title%</a>
                                    Reason: '%reason%'
                                    Message is given below.
                                    %message%
    ...
    public function prepareAdminEmailJobCancelled($email_params)
    {
        ...
        $email_content_default =    "Hello,
                                    An Employer <a href=' %employer_link%'>%employer_name%</a> has been cancelled his ongoing project <a href='%project_link%'>%resource_title%</a> assign to <a href='%student_link%'> %student_name% </a>
